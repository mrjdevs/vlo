...
use axum::{
    ...
    response::{
        sse::{Event, KeepAlive},
        Html, IntoResponse, Json, Redirect, Sse,
    },
    routing::get,
    Router,
};
...
static DIRECTIVE_DEL_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
...
static DIRECTIVE_PUT_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
...
static CONFIRM_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"(?is)v-confirm\s*=\s*["']([^"']*)["']"#).unwrap());

static PARAM_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"(?is)v-param\s*=\s*["']([^"']+)["']"#).unwrap());

static PROMPT_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"(?is)v-prompt\s*=\s*["']([^"']*)["']"#).unwrap());

static DIRECTIVE_ATTR_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
...
static DATA_SOURCE_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
...
static EMPTY_TAG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)<(h[36]|p|strong)(\s+[^>]*)?>\s*</(h[36]|p|strong)>").unwrap()
});
...
fn read_component_template(
    ...
    let template =
        STYLE_RE.replace_all(&source, "").into_owned();
...
fn render_tag(
    ...
    if let Some((start, end, props, children)) =
        find_tag(source, tag)
    {
        return format!(
            "{}{}{}",
...
fn render_components(
    ...
    while index < chars.len() {
                ...
                .is_ascii_uppercase()
        {
            ...
            let tag =
                &source[chars[index + 1].0..chars[end].0];
...
fn render_component_template(
    ...
    let rendered = clean_empty_tags(&rendered);
...
fn clean_empty_tags(html: &str) -> String {
    ...
    loop {
        let result = EMPTY_TAG_RE.replace_all(&cleaned, "");
        if result == cleaned {
            break;
        }
...
fn resolve_data_sources(
    ...
    for cap in DATA_SOURCE_RE.captures_iter(source) {
        let full =
            cap.get(0).unwrap();
...
fn evaluate_data_source_block(
    inner: &str,
    action: &str,
) -> String {
    let data =
        fetch_api_data_sync(action);
...
fn render_vlo(
    ...
    let mut source =
        strip_server_block(&source);

    for captures in
        STYLE_RE.captures_iter(&source)
    {
        if let Some(style) =
            captures.get(1)
        {
...
// ============================================================
// 21. VLO STATIC FILE SERVER
// ============================================================

// Static assets served via tower-http ServeDir with compression layer in dev().
...
async fn dev(
    ...
    println!(
        "⚡ VLO v0.7 dev server running at http://{}",
        addr
    );
...
fn build() {
        ...
        fs::read_dir(&pages)
    {
            ...
            entries.flatten()
        {
            ...
            .expect(
                "Failed to write generated HTML",
            );
...
fn strip_vlo_directive_attrs(
    attrs: &str,
) -> String {
    DIRECTIVE_ATTR_RE.replace_all(attrs, "").into_owned()
}
...
fn init_project(
    ...
    let pages_dir =
        target_dir.join("pages");

    let api_dir =
        pages_dir.join("api");

    let layouts_dir =
        target_dir.join("layouts");

    let components_dir =
        target_dir.join("components");

    let public_dir =
        target_dir.join("public");
...
<style>
    ...
    .app-footer { border-top: 1px solid #334155; padding-top: 1.5rem; margin-top: 3rem; text-align: center; color: #64748b; font-size: 0.875rem; }
        ...
        .expect(
            "Failed to write BaseLayout.vlo",
        );
    }
