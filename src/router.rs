...
pub fn evaluate_data_source_block(inner: &str, action: &str) -> String {
...
pub async fn page_handler(AxumPath(path): AxumPath<String>) -> impl IntoResponse {
...
pub async fn render_page(path: String, dev: bool) -> impl IntoResponse {
...
pub fn wrap_html(title: &str, rendered: &RenderedPage, dev: bool) -> String {
    let hmr = if dev {
        r#"<script>
        ...
        if let Ok(mut timestamp) = last.try_lock() {
            if timestamp.elapsed().as_millis() > 200 {
                *timestamp = Instant::now();
                if let Ok(mut cache) = crate::state::TEMPLATE_CACHE.lock() {
                    cache.clear();
                }
                let _ = tx.send(());
                println!("⚡ Reload");
